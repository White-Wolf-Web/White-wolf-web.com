export const DataCardHomePage = [
	{
		card1: {
			id: "1",
			imgSrc: "/Assets/Images/Webp/webiconblue.webp",
			title: "Création de sites web &\n e-commerce personnalisés 🌐🛍️",
			titleAlt: "le monde du web et ses sites",
			text: "Notre agence se spécialise dans la conception et le développement de sites web et de boutiques en ligne attrayants, fonctionnels et faciles à utiliser. Nous mettons l'accent sur l'expérience utilisateur (UX) et le design pour créer des sites web qui attirent l'attention et encouragent les visiteurs à devenir des clients fidèles. Que vous ayez besoin d'un site vitrine pour présenter votre entreprise ou que vous ayez un projet d'une boutique en ligne complète, notre équipe est là pour transformer vos idées en réalité et à optimiser son référencement pour attirer plus de clients.",
			link: "/articles/creation-site-web",
		},
		card2: {
			id: "2",
			imgSrc: "/Assets/Images/Webp/paintPalette.webp",
			title: "Design et ergonomie 🎨✨",
			titleAlt: "Palette de peinture utilisé pour le web-design",
			text: "Le design est un élément clé de la réussite de votre site web. Chez White Wolf Web, nous sommes convaincus qu'un design attrayant et une ergonomie bien pensée peuvent faire la différence entre un site web qui convertit et un site internet qui échoue. Nos designers talentueux travaillent en étroite collaboration avec nos développeurs pour créer des sites web qui reflètent votre image de marque et offrent une expérience utilisateur exceptionnelle. Cela permet de captiver votre visiteur, nous savons qu'une image attrayante et de qualité peut faire la différence sur votre site internet.",
			link: "/articles/web-design",
		},
		card3: {
			id: "3",
			imgSrc: "/Assets/Images/Webp/seo.webp",
			title: "Référencement et SEO 🔍📈",
			titleAlt: "Petits en rapport au réferencement naturel ou SEO",
			text: "Un bon positionnement dans les moteurs de recherche tels que Google, Bing, Yandex ... est essentiel pour attirer du trafic sur votre site internet. Notre équipe d'experts en référencement travaille sans relâche pour optimiser votre site web et améliorer sa visibilité en ligne. Nous utilisons des techniques de SEO éprouvées et des outils d'analyse avancés pour garantir que votre site internet atteigne les premières positions des résultats de recherche.",
			link: "/articles/referencement-seo-google",
		},
		card4: {
			id: "4",
			imgSrc: "/Assets/Images/Webp/Social_network_logos.webp",
			title: "Marketing digital et\n réseaux sociaux 📣🌐",
			titleAlt: "Facebook, Instagram, Twitter, Pinterest, Youtube ...",
			text: "Pour assurer la réussite de votre entreprise en ligne, il est crucial de mettre en place une stratégie de marketing digital efficace. Nos experts en marketing digital vous aideront à développer votre présence en ligne et à atteindre votre cible grâce à des campagnes publicitaires, du content marketing et une gestion efficace des réseaux sociaux (Facebook, Instagram, Twitter ...).",
			link: "/articles/reseaux-sociaux",
		},
		card5: {
			id: "5",
			imgSrc: "/Assets/Images/Webp/Clipboard.webp",
			title: "Stratégie de contenu et\n rédaction 💡📝",
			titleAlt: "Une page vierge un crayon et une gomme ... l'envie d'écrire",
			text: "Le contenu de qualité est essentiel pour attirer et fidéliser votre audience. Notre équipe de rédacteurs expérimentés et créatifs élabore des stratégies de contenu personnalisées pour votre entreprise, en tenant compte de votre public cible et de vos objectifs. Nous produisons du contenu engageant, informatif et optimisé pour le référencement, qui vous aidera à vous démarquer de la concurrence et à améliorer votre positionnement en ligne sur les moteurs de recherche tel que Google.",
			link: "/articles/miscellaneous/contenu-et-redaction-votre-site-internet",
		},
		card6: {
			id: "6",
			imgSrc: "/Assets/Images/Webp/wordpress5.webp",
			title: "Création de site Wordpress",
			titleAlt: "Le fameux W qui symbolise la création des sites web Wordpress",
			text: "WordPress est l'une plateforme de création de sites web les plus populaires et les plus polyvalents. Selon les besoins de votre site web d'entreprise, de votre blog ou de votre boutique en ligne … Vous pourrez y ajouter des fonctionnalités de pointe tels que des plugins, un template (selon le thème), le blog ... tout en vous assurant une navigation fluide pour vos visiteurs.",
			link: "/articles/creation-site-web/wordpress",
		},
		card7: {
			id: "7",
			imgSrc: "/Assets/Images/Webp/fleche-montante.webp",
			title: "Analyse et\nsuivi des performances 📊🔍",
			titleAlt: "C'est une graphisme qui permet de suivre les évolutions des performance du site internet",
			text: "Chez White Wolf Web, nous comprenons l'importance de mesurer et d'analyser les performances de votre site internet et de vos campagnes marketing. Notre équipe d'analystes utilise des outils avancés pour suivre et analyser les données, afin de vous fournir des rapports détaillés et des recommandations pour améliorer continuellement votre stratégie en ligne. Notre objectif est de vous aider à prendre des décisions éclairées et à maximiser le retour sur investissement de vos efforts en ligne.",
			link: "/articles/miscellaneous/analyse-audit-performance-votre-site-web",
		},
		
	},
];
